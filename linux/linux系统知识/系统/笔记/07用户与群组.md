...
### 用户相关命令
**增删改**
>- useradd:  添加用户[[07用户与群组#etc default useradd|4]]
>- [[07用户与群组#userdel|userdel]]:　删除用户
>- usermod:　修改用户信息
>- passwd：修改用户密码
>- chsh：　修改用户默认bash [[07用户与群组#nologin与false|1]] [[01shell#shell类型与环境变量|2]]
...
**切换用户及相关**
>- [[07用户与群组#su|su]]：　切换用户
>- [[07用户与群组#sudo|sudo]]：　使用root权限[[07用户与群组#添加sudo命令|3]]
>- [[07用户与群组#visudo|visudo]]：修改[[07用户与群组#sudoers文件|/etc/sudoers]]文件
...
>- groupadd:　添加组
>- groupdel：　删除组
>- [[07用户与群组#groupmod|groupmod]]：更改群组的识别码或名称
>- [[07用户与群组#groupmems|groupmems]]：用户主组的管理员成员
>- [[07用户与群组#newgrp|newgrp]]:修改用户[[07用户与群组#有效群组|有效群组]]
...
### 配置文件
**用户**
>- [[07用户与群组#etc passwd|/etc/passwd]]　:用户配置文件
>- [[07用户与群组#etc shadow|/etc/shadow]]	   :用户密码
>- [[07用户与群组#etc login defs|/etc/login.defs]]:用户属性限制,密码过期时间,密码最大长度等限制
>- [[07用户与群组#etc default useradd|/etc/default/useradd]]:默认的useradd配置文件

**组**
>- [[07用户与群组#etc group|/etc/group]]	　   :组配置文件
>- [[07用户与群组#etc gshadow|/etc/gshadow]]　   :组密码

**sudo命令配置文件**
>- [[07用户与群组#sudoers文件|/etc/sudoers]]
...
而且都设置了[[03权限控制#SUID|SUID]] ，这也能解释了sudo提权的方式。
...
可以看到这个文件是只读的　如果想要修改　建议使用[[07用户与群组#visudo|visudo]]
...
<br></br>
##### 初始群组
[[07用户与群组#etc passwd|/etc/passwd]]下第四个字段代表的是组id，这个id就是初始群组，通过[[07用户与群组#newgrp|newgrep]]命令可以临时改变初始群组
```bash
uos@uos-PC:~$ groups
uos root lp sudo users netdev lpadmin scanner sambashare
uos@uos-PC:~$ 
